-- Migration 010 — Alinhar URLs de imagens aos slugs do seed 001_products.sql
-- Problema: migrations 007–009 usavam WHERE slug = '...-de-...' enquanto o seed usa
-- slugs curtos (ex.: suporte-plantas-hexagonal vs suporte-de-plantas-hexagonal).
-- Os UPDATE não batiam linhas → muchos produtos ficaram só com placeholder.
--
-- Também garante uma imagem principal para qualquer produto sem linha em product_images.
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'suporte-plantas-hexagonal');

UPDATE product_images SET url = 'https://images.unsplash.com/photo-1553556135-009e5858adce?w=600&h=600&fit=crop&crop=center'
WHERE product_id = (SELECT id FROM products WHERE slug = 'suporte-celular-ajustavel');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'organizador-cabos-clip');

UPDATE product_images SET url = 'https://picsum.photos/seed/kitchen-trivet/600/600'
WHERE product_id = (SELECT id FROM products WHERE slug = 'descanso-panela-modular');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'organizador-mesa-premium');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'suporte-fone-gamer');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'porta-cartoes-visita');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'suporte-controle-universal');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'organizador-pinceis-maquiagem');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'suporte-giratorio-batons');
...
WHERE product_id = (SELECT id FROM products WHERE slug = 'espelho-mesa-aro-decorativo');
...
SELECT
  p.id,
  'https://placehold.co/600x600/1a1a2e/b683ff?text=' || replace(replace(p.name, ' ', '+'), '&', ''),
  p.name,
  true,
  0
FROM products p
